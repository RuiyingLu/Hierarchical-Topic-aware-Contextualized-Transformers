...
parser.add_argument('--run_name', type=str, default=run_name, help='Run id. Name of subdirectory in checkpoint/ and samples/')
parser.add_argument('--sample_every', metavar='N', type=int, default=1000, help='Generate samples every N steps')
parser.add_argument('--sample_length', metavar='TOKENS', type=int, default=1024, help='Sample this many tokens')
...
parser.add_argument('--test_every', metavar='STEPS', type=int, default=200, help='Calculate validation loss every STEPS steps.')
...
parser.add_argument('--tm_learning_rate',type=int, default=0.0001, help='learning_rate of pretrained topic model')
...
parser.add_argument('--phi_save_path',type=str, default=phi_save, help='phi_save_path')
...
def main():
    ...
    with tf.Session(config=config) as sess:
        ...
        Phi_tensor_1 = tf.transpose(tf.convert_to_tensor(Phi[0].astype('float32')))
        Phi_tensor_2 = tf.transpose(tf.convert_to_tensor(Phi[1].astype('float32')))
        Phi_tensor_3 = tf.transpose(tf.convert_to_tensor(Phi[2].astype('float32')))

        gbn3 = gbn_3_model(len(Phi[0]), args, len(docs_bow))

        theta = [tf.reshape(gbn3.theta1/tf.reduce_sum(gbn3.theta1,axis=0,keep_dims=True),[args.theta_size[0],args.batch_size,args.train_sequence_len]),
                 tf.reshape(gbn3.theta2/tf.reduce_sum(gbn3.theta2,axis=0,keep_dims=True),[args.theta_size[1],args.batch_size,args.train_sequence_len]),
                 tf.reshape(gbn3.theta3/tf.reduce_sum(gbn3.theta3,axis=0,keep_dims=True),[args.theta_size[2],args.batch_size,args.train_sequence_len])]

        output = model.model_layer3_virtual(hparams=hparams, X=context_in, Theta = theta, Phi = [Phi_tensor_1,Phi_tensor_2,Phi_tensor_3])
        ...
        if args.val_every > 0:
            ...
            val_theta = [tf.reshape(gbn3.theta1 / tf.reduce_sum(gbn3.theta1, axis=0, keep_dims=True),
                                [args.theta_size[0], args.batch_size, args.train_sequence_len]),
                     tf.reshape(gbn3.theta2 / tf.reduce_sum(gbn3.theta2, axis=0, keep_dims=True),
                                [args.theta_size[1], args.batch_size, args.train_sequence_len]),
                     tf.reshape(gbn3.theta3 / tf.reduce_sum(gbn3.theta3, axis=0, keep_dims=True),
                                [args.theta_size[2], args.batch_size, args.train_sequence_len])]
            val_output = model.model_layer3_virtual(hparams=hparams, X=val_context, Theta=val_theta,
                                                 Phi=[Phi_tensor_1, Phi_tensor_2, Phi_tensor_3])  ###theta
        ...
        if args.test_every > 0:
            ...
            test_theta = [tf.reshape(gbn3.theta1 / tf.reduce_sum(gbn3.theta1, axis=0, keep_dims=True),
                                [args.theta_size[0], args.batch_size, args.train_sequence_len]),
                     tf.reshape(gbn3.theta2 / tf.reduce_sum(gbn3.theta2, axis=0, keep_dims=True),
                                [args.theta_size[1], args.batch_size, args.train_sequence_len]),
                     tf.reshape(gbn3.theta3 / tf.reduce_sum(gbn3.theta3, axis=0, keep_dims=True),
                                [args.theta_size[2], args.batch_size, args.train_sequence_len])]
            test_output = model.model_layer3_virtual(hparams=hparams, X=test_context, Theta=test_theta,
                                                 Phi=[Phi_tensor_1, Phi_tensor_2, Phi_tensor_3])  ###theta
        ...
        train_vars = [v for v in train_vars if 'topic' not in v.name]
        new_vars=all_vars[2:5]
        ...
        try:
            while True:
                ...
                if args.accumulate_gradients > 1:
                    ...
                    print(np.max(train_batch_gbn))
                    (_, v_loss, v_summary, theta_temp) = sess.run(
                        (opt_apply, loss, summaries,theta),
